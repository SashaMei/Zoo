	...
	public static void main(String[] args) {
		...
		for(Animal animal : animalList) {
			if(animal instanceof Winged) {
				System.out.println((Winged)animal);
			}
		}
		...
		for(Animal animal : animalList) {
			if(animal instanceof Adoptable) {
				System.out.println((Adoptable)animal+ ((Adoptable)animal).homeCareDirections());
			}
		}
		...
		for(Animal animal : animalList) {
			if(animal instanceof WaterDweller) {
				System.out.println((WaterDweller)animal+ (((WaterDweller)animal).canLiveOutOfWater() ? "\tCan also live on land": "\tCannot also live on land" ));
			}
		}
